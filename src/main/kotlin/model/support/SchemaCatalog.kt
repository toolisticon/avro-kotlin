...
internal data class SchemaCatalog(
  private val map: Map<AvroHashCode, NamedSchema> = mapOf(),
  val graph: Graph<AvroHashCode> = Graph()
) : Map<AvroHashCode, NamedSchema> by map {
  companion object {

    fun bySchemaType(schemaType: SchemaType): (Map.Entry<AvroHashCode, NamedSchema>) -> Boolean = { schemaType.get() == it.value.schema.type }
   ...
   */
  constructor(namedSchema: NamedSchema) : this(SchemaCatalog() + namedSchema)

  /**
   * Creates new instance based on given schemas.
   */
  constructor(namedSchemas: List<NamedSchema>) : this(SchemaCatalog() + namedSchemas)
   ...
   * Uses plus(Schema)
   */
  operator fun plus(namedSchemas: List<NamedSchema>) = namedSchemas.fold(this) { a, c -> a + c }

  /**
   * Adds a single schema and all of its enclosed types to the current catalog.
   */
  operator fun plus(namedSchema: NamedSchema): SchemaCatalog {
    val hashCode = AvroHashCode(namedSchema.schema)

    return if (contains(hashCode)) {
      this
    } else {
      val enclosed = namedSchema.schema.enclosed()

      copy(
        map = map + mapOf(hashCode to namedSchema),
        graph = graph + hashCode + enclosed.map { hashCode to AvroHashCode(it.schema) }
      ).plus(enclosed)
    }
  }
}
 ...
 */
internal fun SchemaCatalog.single(schemaType: SchemaType): Pair<AvroHashCode, NamedSchema> = filter(bySchemaType(schemaType)).entries.single().toPair()
