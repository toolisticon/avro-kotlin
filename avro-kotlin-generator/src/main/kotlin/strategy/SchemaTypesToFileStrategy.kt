...
class SchemaTypesToFileStrategy : AvroFileSpecFromSchemaDeclarationStrategy() {
  ...
  override fun invoke(context: SchemaDeclarationContext, input: SchemaDeclaration): KotlinFileSpec {
    ...
    val fileBuilder = fileBuilder(className = className).apply {
      addAnnotation(GeneratedAnnotation(AvroKotlinGenerator.NAME).date(context.properties.nowSupplier()))
    }
    ...
    val rootDataClassBuilder = dataClassBuilder(className).apply {
      ...
      context.registry.processors.filter(KotlinDataClassFromRecordTypeProcessorBase::class).executeAll(context, input.recordType, this)
